...
首次安装会打开设置页。词典查询无需配置；翻译方案可按优先级添加 DeepL、Google Cloud、百度翻译、火山引擎或兼容 OpenAI 的 AI 接口，也可以使用内置的免密钥网页翻译：必应翻译、Google 翻译、MyMemory、Yandex 和 Reverso。新装默认方案链为 必应翻译 → Google 翻译 → MyMemory → Yandex → Reverso，开箱即用；这五个内置方案带「网页版」徽标，固定在默认列表中，不可编辑或删除，添加翻译方案的下拉中也不再重复提供。

内置网页方案无需注册密钥：必应翻译与 Google 翻译复用对应网页版接口，源语言自动检测，目标语言跟随设置页选择；MyMemory 调用其公开翻译记忆接口，单次输入最多 500 字节（按 UTF-8 计）；Yandex 复用其浏览器客户端接口，目标语言与 Google 基本一致但不支持繁體中文；Reverso 免密接口按文字系统在本地识别源语言，支持 15 种目标语言，同样不支持繁體中文。免费接口没有官方可用性保证，偶有限流或接口变更，某个方案失败后会进入 10 分钟冷却，翻译自动顺延到方案链中的下一个方案；冷却期间手动点击「测试」成功会立即解除该方案的冷却。
