...
首次安装会打开设置页。词典查询无需配置；翻译方案可按优先级添加 DeepL、Google 翻译、Google Cloud、百度翻译、火山引擎或兼容 OpenAI 的 AI 接口，也可以使用免密钥的网页方案：百度网页翻译、必应翻译、腾讯翻译、有道翻译和彩云小译。新装默认方案链为 Google 翻译 → 百度网页翻译 → 必应翻译 → 腾讯翻译，开箱即用。

免费网页方案无需注册密钥：百度网页翻译、必应翻译、腾讯翻译、有道翻译复用对应网页版接口，源语言自动检测，目标语言跟随设置页选择；彩云小译调用其公开接口，仅支持译为简体中文。免费接口没有官方可用性保证，偶有限流或接口变更，某个方案失败时翻译会自动顺延到方案链中的下一个方案。
